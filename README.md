...
- **Kanban view** — Open / Urgent / Done columns with a "Diese Woche" progress bar tracking actual completions (via a real completion date, not a due-date proxy) against this calendar week's workload; the column grouping itself is unchanged, and only overdue cards carry a red accent
- **Heute / Diese Woche** — a flat, cross-project work surface: overdue and due-today tasks separated from the rest of the calendar week, plus a project-less "Ohne Projekt" bucket for tasks with no project relation in Notion. Inline complete/reschedule reuse the same task API as every other view
