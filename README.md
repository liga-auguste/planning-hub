...
Every Claude call runs inside the request — no task queue, no background workers. The Sonnet call blocks the thread serving it for its duration, and that is a deliberate choice: there is one user, the demo is nginx-rate-limited to 30 requests per minute (burst 10), and a Claude call is spent waiting on an API rather than computing. A task queue would add a broker, a worker process and a result store to solve a concurrency problem that does not exist. While a call runs, the UI shows a loading state and blocks double submits.
...
That only stays cheap because a confirmed write patches the cache instead of invalidating it. A toggle and a rename move no task (`done` and the name are deliberately not part of the sort key), so the cached lists and the AI summary's references both survive; a reschedule does move one, so the projects are re-sorted and only the summary is dropped. A removal is the one write with no patch path — it shifts every count and every summary reference at once — so it falls back to the full invalidation deliberately, for the rarest write in the app. Where a patch cannot be applied safely — a cold cache, a task no cached list carries — the code falls back to the full invalidation it replaced. A patch also never renews the entry: the eight-hour TTL is a policy about how stale a *Notion read* may get, so the read stamps the deadline and every later write keeps it. Renewing it on each checkbox would mean a task edited in Notion's own UI never surfacing, for as long as the dashboard kept being used. Full writeup: [`docs/dashboard-write-paths.md`](docs/dashboard-write-paths.md).
