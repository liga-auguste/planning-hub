...
Because the suite needs nothing beyond the code, `.github/workflows/test.yml` runs it on every push and pull request — on Python 3.12 to match the `Dockerfile`, with `DEMO_MODE=true` so SQLite is used and no Postgres service or API key is required.
