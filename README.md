...
- **Kanban view** — Open / Urgent / Done columns with a "Diese Woche" progress bar tracking actual completions (via a real completion date, not a due-date proxy) against this calendar week's workload; the column grouping itself is unchanged, and only overdue cards carry a red accent. The bar counts only project-linked tasks, matching what the board beneath it can actually render — the project-less "Ohne Projekt" bucket is tracked separately, in the Heute view below
